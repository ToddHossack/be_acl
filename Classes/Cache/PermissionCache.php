...
class PermissionCache implements SingletonInterface {
	...
	/**
	 * @var \Tx\BeAcl\Cache\TimestampUtility
	 */
	 ...
	 *
	 * @param \TYPO3\CMS\Core\Authentication\BackendUserAuthentication $backendUser The Backend user for which the cache is managed
	 */
	...
	public function setPermissionsClause($requestedPermissions, $permissionsClause) {
		...
		if (!$this->isValidCacheData($cacheData)) {
			$cacheData = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('Tx\\BeAcl\\Cache\\PermissionCacheData');
		}
	...
	protected function initializeRequiredClasses() {
		...
		$timestampUtility = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('Tx\\BeAcl\\Cache\\TimestampUtility');
