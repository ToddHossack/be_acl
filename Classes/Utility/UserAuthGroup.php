...
class UserAuthGroup {
	...
	public function calcPerms($params, $that) {
		...
		foreach ($rootLine as $values) {
			...
			$res = $this->db->exec_SELECTquery('*', 'tx_beacl_acl', 'pid=' . intval($values['uid']) . $recursive, '', 'recursive ASC');
	...
	public function getPagePermsClause($params, $that) {
		...
		$permissionCache = GeneralUtility::makeInstance('Tx\\BeAcl\\Cache\\PermissionCache');
