	...
	function select($PA, $fobj) {
		...
		switch ($type) {
			...
			case '0':
				...
				if (!$GLOBALS['BE_USER']->isAdmin()) $items = BackendUtility::blindUserNames($items, $GLOBALS['BE_USER']->userGroupsUID, 1);
			...
			case '1':
				...
				if (!$GLOBALS['BE_USER']->isAdmin()) $items = BackendUtility::blindGroupNames($items, $GLOBALS['BE_USER']->userGroupsUID, 1);
